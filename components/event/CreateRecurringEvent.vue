<template>
  <Dialog 
    ...
    :closable="true"
    :closeOnEscape="true"
    class="create-recurring-event-dialog"
  >
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <div>
          <h3 class="text-xl font-semibold text-text-main">Create Recurring Event</h3>
          <p class="text-sm text-text-muted">Schedule a recurring food truck event</p>
        </div>
      </div>
    </template>
    ...
    <template #footer>
      <div class="flex justify-end gap-3">
        ...
        <Button 
          ...
          :loading="loading" 
          :disabled="!canCreateRecurringEvent"
...
interface Emits {
  ...
  (e: 'recurring-event-created'): void
}
...
const scheduleIntervalUnit = ref('days')
...
watch(() => props.preFilledDate, (newDate: Date | undefined) => {
  if (newDate) {
...
const closeDialog = () => {
  ...
  recurrenceDayOfMonth.value = null
...
const createRecurringEvent = async () => {
  ...
  try {
    ...
    // Set start and end times if not already set (fallback - should already be set)
    if (!eventStart.value) {
    ...
    // Use store action to create recurring event
    await recurringEventStore.createRecurringEvent(recurringEventObj)
...
:deep(.create-recurring-event-dialog .p-inputnumber .p-inputnumber-input) {
  width: 100% !important;
...
:deep(.create-recurring-event-dialog .p-inputnumber .p-inputnumber-input-group) {
  width: 100% !important;
  max-width: 100% !important;
  min-width: 0 !important;
}

:deep(.create-recurring-event-dialog .p-inputnumber-button-group) {
  display: none !important;
  width: 0 !important;
}

:deep(.create-recurring-event-dialog .p-inputnumber .p-inputnumber-button) {
  display: none !important;
  width: 0 !important;
  height: 0 !important;
  padding: 0 !important;
